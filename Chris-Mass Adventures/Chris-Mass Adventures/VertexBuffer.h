...
{
	template <typename VertexFormat> // is this needed ?
	...
	public:
		...
		inline ID3D11Buffer * const GetVertexBuffer() { return vertexBuffer; };
	private:
	...
	template <typename VertexFormat>
		UINT VertexBuffer<VertexFormat>::AddVerts(const VertexFormat *verts, UINT _numVerts)
		{
			...
			if (!vertexBufferPtr)
			{
				...
				Renderer::theDevicePtr->CreateBuffer(&vbd, &vinitData, &vertexBufferPtr);
			}
			else
			{
